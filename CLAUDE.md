...
7. ~~Add `<url>` entry to `sitemap.xml`~~ — **SKIP.** `sitemap.xml` is main-side single-writer (SO #35; `derived-artifacts-regen.yml` COVERED id `sitemap-xml`, SITEMAP-MAINSIDE-1, 2026-08-29). A PR never edits it; `derived-artifacts-regen.yml` regenerates and commits it on `main` after merge.
...
0b. **Hub freshness** — `chaingraph/chaingraph-hub.html` is main-side single-writer (SO #35; COVERED ids `chain-index`, `chaingraph-hub`). Do **NOT** run `gen-chain-index.mjs`/commit it in your PR — `derived-artifacts-regen.yml` regenerates it after merge. `node scripts/gen-chain-index.mjs --check` runs ADVISORY on a PR and BLOCKING on `main`; a red PR-side result just tells you main hasn't caught up yet, not that you owe a fix.
1. **Manifests** — one per new tool: `manifests/{number}-{slug}.manifest.json`. No short-form `{number}-manifest.json` variants.
2. **Catalog** — `mcp/catalog.json`, `mcp/server.json`, `.well-known/mcp.json`, `llms.txt`, `tools.html`, and `index.html` are main-side single-writer (SO #35; COVERED id `catalog`, generator `python scripts/regen_catalog.py`). **Do NOT run or commit `regen_catalog.py` in your PR** — `derived-artifacts-regen.yml` runs it after merge. `docs/catalog.json` is a copy of `mcp/catalog.json` written later by `scripts/gen-openapi.mjs` (also main-side, COVERED id `openapi`), not by this script. The freshness gate for `tools.html`/`index.html`, `python scripts/check_index_sync.py --strict --no-color`, is ADVISORY on a PR and BLOCKING on `main` (SITEMAP-MAINSIDE-1, 2026-08-29) — a red result on your PR means main hasn't regenerated yet, not that your PR must fix it.
3. **Sitemap** — `sitemap.xml` is main-side single-writer (SO #35; COVERED id `sitemap-xml`, SITEMAP-MAINSIDE-1, 2026-08-29). **Do NOT run or commit `node scripts/regen-sitemap.mjs` in your PR** — `derived-artifacts-regen.yml` runs it after merge, scanning every directory in `scripts/published-dirs.json` (the shared manifest `scripts/verify_repo.py`'s `check_sitemap` reads too, so generator and gate cannot scope-drift). Gate `node scripts/regen-sitemap.mjs --check` is ADVISORY on a PR, BLOCKING on `main`. **Not `scripts/regen_sitemap.py`**, now quarantined as `scripts/regen_sitemap.py.DEPRECATED`: it knew only `tools/`, `guides/` and `chaingraph/` plus seven hardcoded root pages, so it silently dropped every published page outside that scope and turned the freshness gate red.
...
7. **Stat sync** — `mcp.html`/`chaingraph/chaingraph-hub.html`'s workflow/tool-count fallbacks are main-side single-writer (SO #35; COVERED id `stats`, generator `node scripts/sync-stats.mjs --fix`). **Do NOT run `--fix` and commit it in your PR** — `derived-artifacts-regen.yml` runs it after merge. `node scripts/sync-stats.mjs` (no flag) is ADVISORY on a PR, BLOCKING on `main`.
8. **Count-drift gate** — every published count sentinel (`<!--COUNT:key-->N<!--/COUNT-->`, `data-count="key"`) is main-side single-writer (SO #35; COVERED id `counts`, generator `node scripts/verify-counts.mjs --fix`). **Do NOT run `--fix` and commit it in your PR** — `derived-artifacts-regen.yml` runs it after merge. `node scripts/verify-counts.mjs --check` is ADVISORY on a PR, BLOCKING on `main`. **Never hardcode a count to make this pass — fix the sentinel or the source**, and let main's regen repair the derived value.
