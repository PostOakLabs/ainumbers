...
2. **Catalog** — `python scripts/regen_catalog.py` — regenerates `mcp/catalog.json`, `mcp/server.json`, `.well-known/mcp.json`, `llms.txt`, `tools.html`, and `index.html`. `docs/catalog.json` is a copy of `mcp/catalog.json` written later by `scripts/gen-openapi.mjs`, not by this script.
