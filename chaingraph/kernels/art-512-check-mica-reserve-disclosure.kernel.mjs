...
export function compute(pp) {
  ...
  rationale.push(eligibleDeclared
    ? (ineligible_components.length > 0
        ? `${ineligible_components.length} component${ineligible_components.length === 1 ? '' : 's'} fall outside the declared eligible classes and are listed individually; their amounts remain inside the reserve total, because excluding them would flatter the coverage figure.`
  ...
  rationale.push(concentration_breaches.length > 0
    ? `${concentration_breaches.length} asset class${concentration_breaches.length === 1 ? '' : 'es'} exceed the declared concentration limit, each named with its excess.`
  ...
  rationale.push(custodiansDeclared
    ? (undeclared_custodian_components.length > 0
        ? `${undeclared_custodian_components.length} component${undeclared_custodian_components.length === 1 ? '' : 's'} sit with a custodian type the caller did not declare acceptable, each named.`
  ...
  rationale.push(!cadenceDeclared
    ...
    : (missed_periods.length > 0
        ? `${missed_periods.length} period${missed_periods.length === 1 ? '' : 's'} ran longer than the declared ${cadenceDays}-day cadence with no publication, each named with its dates.`
