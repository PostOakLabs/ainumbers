...
// art-636-globe-de-minimis-exclusion — scaffolded by scripts/new-kernel.mjs (KERNEL-SCAFFOLD-1). Fill in:
//   1. compute(pp) below — replace the NOT_IMPLEMENTED throw with real logic.
//   2. chaingraph/kernels/fixtures/art-636-globe-de-minimis-exclusion.fixtures.json — replace the placeholder
//      vector with real golden vectors (name, policy_parameters, output_payload).
//   3. chaingraph/kernels/__proptests__/art-636-globe-de-minimis-exclusion.proptest.mjs — replace the
//      placeholder property with real class-A/B/K properties per FV-PBT-FLOOR-BUILD-SPEC.md.
//   4. chaingraph/graph/nodes/art-636-globe-de-minimis-exclusion.json — replace TODO-CITE / standards_basis and
//      the description once compute() is real.
// Run `node scripts/kernel-preflight.mjs art-636-globe-de-minimis-exclusion` repeatedly while doing all of the
...
export const meta = {
  ...
  mcp_name: 'compute_globe_de_minimis_exclusion',
...
export function compute(pp) {
  pp = pp || {};
  throw new Error('NOT_IMPLEMENTED: art-636-globe-de-minimis-exclusion compute() is a KERNEL-SCAFFOLD-1 stub — replace this with real logic before shipping.');
}
