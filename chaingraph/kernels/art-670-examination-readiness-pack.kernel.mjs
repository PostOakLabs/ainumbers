...
// art-670-examination-readiness-pack — scaffolded by scripts/new-kernel.mjs (KERNEL-SCAFFOLD-1). Fill in:
//   1. compute(pp) below — replace the NOT_IMPLEMENTED throw with real logic.
//   2. chaingraph/kernels/fixtures/art-670-examination-readiness-pack.fixtures.json — replace the placeholder
//      vector with real golden vectors (name, policy_parameters, output_payload).
//   3. chaingraph/kernels/__proptests__/art-670-examination-readiness-pack.proptest.mjs — replace the
//      placeholder property with real class-A/B/K properties per FV-PBT-FLOOR-BUILD-SPEC.md.
//   4. chaingraph/graph/nodes/art-670-examination-readiness-pack.json — replace TODO-CITE / standards_basis and
//      the description once compute() is real.
// Run `node scripts/kernel-preflight.mjs art-670-examination-readiness-pack` repeatedly while doing all of the
...
export function compute(pp) {
  pp = pp || {};
  throw new Error('NOT_IMPLEMENTED: art-670-examination-readiness-pack compute() is a KERNEL-SCAFFOLD-1 stub — replace this with real logic before shipping.');
}
