  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    for (const fs of e.applies_to_filer_statuses) {
      if (FILER_STATUSES.indexOf(fs) === -1) {
        return { ok: false, error_code: 'SLICE_INVALID_FILER_STATUS', message: `entries[${i}] ("${e.standard_id}") declares filer_status "${String(fs)}", which is not in the closed enum` };
      }
...
function validateQuery(query) {
  ...
  if (parseISODate(query.fiscal_year_end) === null) {
    return {
      ...
      message: `query.fiscal_year_end "${String(query.fiscal_year_end)}" is not a valid ISO calendar date — it is a DATE, never a month/day pair, because 52/53-week filers' year-ends move`,
    };
  }
  if (FILER_STATUSES.indexOf(query.filer_status) === -1) {
    return {
      ok: false, error_code: 'QUERY_INVALID_FILER_STATUS',
      message: `query.filer_status "${String(query.filer_status)}" is not in the closed enum {${FILER_STATUSES.join(', ')}}`,
    };
  }
  ...
  if (query.fiscal_year_begin !== undefined && query.fiscal_year_begin !== null && parseISODate(query.fiscal_year_begin) === null) {
    return { ok: false, error_code: 'QUERY_INVALID_FISCAL_YEAR_BEGIN', message: `query.fiscal_year_begin "${String(query.fiscal_year_begin)}" is neither absent nor a valid ISO calendar date` };
  }
...
function resolve(pp) {
  ...
  if (declared === null || declared !== recomputed.toLowerCase()) {
    return _nullResult({
      ...
      message: `declared registry_digest "${String(slice.registry_digest)}" does not match the digest "${recomputed}" recomputed over the slice's own bytes`,
