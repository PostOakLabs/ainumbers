...
export const meta = {
  ...
  mcp_name: 'attach_gleif_bundle_enrichment',
...
// copied VERBATIM from art-599 -- kernels are self-contained by construction (no module graph
// beyond _hash.mjs in the zkVM guest), so reuse here means the identical proven
// implementation, not a second algorithm. Do not re-derive it; if art-599 changes, mirror the
// change here.
//
// SCOPING (the actual job of this node, distinct from art-599): a bundle builder states
...
const HIERARCHY_NOTE = 'CORROBORATION, never PROOF (BUNDLE-ENRICH-BUILD-SPEC.md §0): this enrichment is a third-party public record that happened to also capture the same fact, independently of the bundle builder. It strengthens a narrative but is never load-bearing -- the bundle\'s PROOF remains its own hash + RFC3161/OTS timestamp anchor, unaffected by whether this enrichment is present, absent, or inapplicable.';
const RECURRING_DUTY_NOTE = 'No recurring duty: one pin per bundle-build invocation, only for bundle types the builder has declared LEI-bearing. No scheduler, no freshness promise, nothing to keep current afterwards.';

// --- ISO 17442 LEI check-digit validation (verbatim from art-599, itself verbatim from
// art-246 -- ISO 7064 Mod 97-10). Do not re-derive it. ---------------------------------------
...
// --- Pure-JS SHA-256 (sync), verbatim from art-599 (byte-identical to WebCrypto; inlined so
// compute() stays synchronous and runs in the zkVM guest). -----------------------------------
...
// Verbatim from art-599: LastUpdateDate is read ONLY from the unambiguous CDF XML element.
...
export function compute(pp) {
  ...
  const bundle_type = str(pp.bundle_type).trim() || null;
  const declared = triState(pp.bundle_asserts_lei_identity);
  const applicable = declared === true;
  ...
  const output_payload = {
    ...
    recurring_duty_note: RECURRING_DUTY_NOTE,
  };
