...
description: "Composes a hash-chained ML training-data lineage record: dataset identity, dataset version, upstream source dataset references, a declared collection/governance method, and an optional reference to an existing OCG receipt for the training-run compute (tool identity, execution hash, kernel digest -- never re-embedded). Chains to a prior lineage record via sha256_prev_lineage_hash. Maps to EU AI Act Art 10 (data and data governance) + Annex IV 2(d) technical-documentation elements; SR 11-7 model risk management data-lineage practice as the US domestic analog. Documents dataset lineage only -- does not validate dataset quality, bias, or representativeness."
