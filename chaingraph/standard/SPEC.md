...
| rendered spec page sub-section parity: every SPEC.md `### §X.Y` heading or `**§X.Y` bold lead-in appears somewhere on the rendered page (SPEC ⊆ page, one-way — the page may render at finer granularity than SPEC states in prose, but SPEC may not lag the page) | `spec-page-subsection-parity.mjs` | validate |
| §24.0 VM↔worker parity: every `gpu:false`, `status:live` kernel's conformance fixtures run in the in-browser QuickJS-ng kernel VM reproduce the fixture `golden_hash` BYTE-FOR-BYTE through the canonical `buildArtifact()` entry (compute() is NOT canonical — two return conventions exist across the corpus) — the VM is a compute surface beside worker/embed/composer/guest, and this gate is its golden-parity equivalent | `vm-parity-gate.mjs --strict` | validate |
| §13 export-format consistency: the `export_artifact` `format` variants (xlsx / csv / pdf / xbrl / vc) describe the SAME underlying evidence for identical input — no format contradicts another on a shared provenance field (audit AUD-C2; runs in the worker repo's CI) | `gate-export-format-consistency.mjs` | validate |
