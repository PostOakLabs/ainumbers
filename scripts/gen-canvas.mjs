...
<div class="cv-grid">
  ...
  <div class="pane-center">
    <div class="cv-toolbar">
      ...
      <a class="cv-btn cv-btn-dim" href="../boundary-explorer.html" style="text-decoration:none;display:inline-flex;align-items:center" title="Sweep a node's numeric inputs to find where its decision flips">&#x1F3AF; Boundary Explorer</a>
    ...
    <div id="rpArtifact" style="display:none">
      ...
      <div class="rp-section">
        ...
        <div class="verify-row">
          ...
          <button class="ex-btn ex-secondary" id="apSendVerifyBtn" onclick="apSendToVerify()" title="Open the Artifact Verifier and hand it this artifact — same-origin message, no file, no network">&#x27A1; Send to Verifier</button>
          <button class="ex-btn ex-secondary" id="apSendSandboxBtn" onclick="apSendToSandbox()" title="Open the Sandbox tamper lab and hand it this artifact — same-origin message, no file, no network">&#x27A1; Send to Sandbox</button>
          <span class="verify-out" id="apVerifyOut"></span>
        </div>
        <div class="verify-out" id="apHandoffOut" style="display:none"></div>
      </div>
      <div class="rp-section">
...
/* ── handoff sender (START-SURFACE-HANDOFF-BUILD-SPEC §1.1/§1.3/§1.4) ──
   Same-origin postMessage only. Never '*'. Payload is the artifact JSON and a
   declared kind, nothing else — no identifiers, no counters, no telemetry.
   Zero network, zero storage: window.open of a same-origin page is neither. */
var HANDOFF_MAX_CHARS = 2000000;
function apHandoffMsg(text, ok) {
  var el = document.getElementById('apHandoffOut');
  if (!el) return;
  el.textContent = text;
  el.className = 'verify-out ' + (ok ? 'v-ok' : 'v-fail');
  el.style.display = text ? 'block' : 'none';
}
function apSendToVerify() {
  if (!currentArtifact) return;
  var json;
  try { json = JSON.stringify(currentArtifact, null, 2); }
  catch (e) { apHandoffMsg('Could not serialize this artifact: ' + e.message, false); return; }
  if (json.length > HANDOFF_MAX_CHARS) {
    apHandoffMsg('Too large to hand off (' + json.length + ' chars, limit ' + HANDOFF_MAX_CHARS + '). Use Export to download the artifact and paste it into the verifier.', false);
    return;
  }
  var target = new URL('../verify.html', window.location.href).href;
  var win = window.open(target, '_blank');
  if (!win) {
    apHandoffMsg('The verifier window was blocked. Allow pop-ups for this site, or download the artifact from Export and paste it in.', false);
    return;
  }
  var sent = false;
  var timer = null;
  function onReady(ev) {
    ...
    win.postMessage({ type: 'ain-handoff/v1', kind: 'ocg-artifact', artifact: json }, window.location.origin);
    apHandoffMsg('Sent to the Artifact Verifier in the new tab.', true);
  }
  window.addEventListener('message', onReady);
  apHandoffMsg('Opening the Artifact Verifier…', true);
  timer = setTimeout(function () {
    if (sent) return;
    window.removeEventListener('message', onReady);
    apHandoffMsg('The verifier did not answer. Download the artifact from Export and paste it in.', false);
  }, 10000);
}

/* ── handoff sender: §1.5 hop D — Workbench/Canvas -> ocg-sandbox.html tamper lab.
   Same discipline as apSendToVerify() above, different target and ready-type check. */
function apSendToSandbox() {
  if (!currentArtifact) return;
  var json;
  try { json = JSON.stringify(currentArtifact, null, 2); }
  catch (e) { apHandoffMsg('Could not serialize this artifact: ' + e.message, false); return; }
  if (json.length > HANDOFF_MAX_CHARS) {
    apHandoffMsg('Too large to hand off (' + json.length + ' chars, limit ' + HANDOFF_MAX_CHARS + '). Use Export to download the artifact and paste it into the sandbox.', false);
    return;
  }
  var target = new URL('../ocg-sandbox.html', window.location.href).href;
  var win = window.open(target, '_blank');
  if (!win) {
    apHandoffMsg('The sandbox window was blocked. Allow pop-ups for this site, or download the artifact from Export and paste it in.', false);
    return;
  }
  var sent = false;
  var timer = null;
  function onReady(ev) {
    ...
    if (timer) clearTimeout(timer);
    win.postMessage({ type: 'ain-handoff/v1', kind: 'ocg-artifact', artifact: json }, window.location.origin);
    apHandoffMsg('Sent to the Sandbox tamper lab in the new tab.', true);
  }
  window.addEventListener('message', onReady);
  apHandoffMsg('Opening the Sandbox…', true);
  timer = setTimeout(function () {
    if (sent) return;
    window.removeEventListener('message', onReady);
    apHandoffMsg('The sandbox did not answer. Download the artifact from Export and paste it in.', false);
  }, 10000);
}
