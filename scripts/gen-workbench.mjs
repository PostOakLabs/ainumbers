...
function apSendToVerify() {
  ...
  if (json.length > HANDOFF_MAX_CHARS) {
    apHandoffMsg('Too large to hand off (' + json.length + ' chars, limit ' + HANDOFF_MAX_CHARS + '). Use the Export section to download the artifact and paste it into the verifier.', false);
    return;
  }
  var target = new URL('../verify.html', window.location.href).href;
  var win = window.open(target, '_blank');
  if (!win) {
    apHandoffMsg('The verifier window was blocked. Allow pop-ups for this site, or download the artifact from the Export section and paste it in.', false);
    return;
  }
  var sent = false;
  var timer = null;
  ...
  timer = setTimeout(function () {
    if (sent) return;
    window.removeEventListener('message', onReady);
    apHandoffMsg('The verifier did not answer. Download the artifact from the Export section and paste it in.', false);
  }, 10000);
}
...
function apSendToSandbox() {
  ...
  if (json.length > HANDOFF_MAX_CHARS) {
    apHandoffMsg('Too large to hand off (' + json.length + ' chars, limit ' + HANDOFF_MAX_CHARS + '). Use the Export section to download the artifact and paste it into the sandbox.', false);
    return;
  }
  var target = new URL('../ocg-sandbox.html', window.location.href).href;
  var win = window.open(target, '_blank');
  if (!win) {
    apHandoffMsg('The sandbox window was blocked. Allow pop-ups for this site, or download the artifact from the Export section and paste it in.', false);
    return;
  }
  var sent = false;
  var timer = null;
  ...
  timer = setTimeout(function () {
    if (sent) return;
    window.removeEventListener('message', onReady);
    apHandoffMsg('The sandbox did not answer. Download the artifact from the Export section and paste it in.', false);
  }, 10000);
}
