...
function statusPill(entry) {
  const state = entry.compute_proof_ready;
  if (state === "ready") return `<span class="status-pill status-ready">proof ready</span>`;
  if (state === "verify_only") return `<span class="status-pill status-verify">verify-only</span>`;
  return `<span class="status-pill status-deferred">proof deferred</span>`;
}
...
.status-verify{background:rgba(148,163,184,.12);color:var(--muted);border:1px solid rgba(148,163,184,.35)}
...
<main>
  <header class="hero" role="banner">
    <div class="container">
      ...
      <p class="hero-sub">Execution proofs cover the computation shown; they do not verify the regulatory interpretation beside them.</p>
    </div>
  </header>
