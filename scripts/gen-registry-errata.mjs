#!/usr/bin/env node
// scripts/gen-registry-errata.mjs — F1 errata/revocation log, published as
// C2SP tlog-tiles-addressed static files under registry/errata/.
// Landed per REGISTRY-ERRATA-TILES-BUILD-1 (REGISTRY-TILES-BUILD-SPEC.md §2,
// §3.2, §3.3, §5, §6). Generated, gate-verified, additive — no liveness duty,
// per Tim's 2026-08-17 ruling (an artefact this generator cannot produce and
// gate-verify end-to-end is not shipped; the row checks off BLOCKED instead).
//
// ⛔⛔ STALENESS (BUILD-SPEC §2.2, verbatim, inherited from FV-AGENTSURFACE-
// SPEC-1 §2 — never re-derive, never paraphrase): "Past `expires_at`, treat
// it as `unknown-refresh-needed` — not as continuing to hold, and not as
// false either." This exact sentence is published as a checkpoint extension
// line (tlog-checkpoint.md's optional extension-lines grammar) below, so it
// ships VERBATIM inside the artifact itself, not just in a comment a reader
// of this source file happens to see.
//
// ── WHAT THIS FILE DOES ──────────────────────────────────────────────────────
// 1. Reads errata.json (repo root — INPUT, owned by FV-ERRATA-BUILD-2/the
//    errata land row, NEVER written by this generator, same read-only
//    relationship this lane has with chaingraph.json, §5.2/5.3). Only its
...
//    ZERO-entry log (errata.json currently ships 0 entries) still produces a
//    valid GENESIS checkpoint at size 0, root = RFC 6962's MTH({}) =
//    SHA-256() (the hash of the empty octet string) — a real, Sigsum-
//    anchored checkpoint that later publishes extend, never a "nothing to
//    do" no-op. No tile/entry-bundle files exist at size 0 (§3.3: empty
//    tiles MUST NOT be served), which is exactly what tileGroups(0) already
//    returns (zero groups) with no special-casing needed.
// 4. Recomputes the PREVIOUS root from tile bytes ALREADY ON DISK (never
//    from a field this run itself would write) and verifies the new tree
//    extends it via verifyConsistency (§2.3, SO #34) — aborts, writes
//    nothing, on failure.
// 5. Formats + signs the checkpoint (Ed25519, log-operator key held OUTSIDE
//    this repo — workspace-root research/registry-errata-log-key.priv.jwk.json)
//    with the §2.2 staleness sentence as an extension line.
...
//    bundles (if any) + checkpoint + Sigsum record to registry/errata/.
//
// `--check` mode (wired into scripts/preflight.mjs) does NONE of the above —
// it re-derives the leaf hashes from the tile/entry bytes ALREADY PUBLISHED
// on disk, recomputes the root, and confirms it matches the currently
// published, signature-verified checkpoint. A single flipped byte in any
// published tile or entry bundle changes the recomputed root and turns this
// red (SO #34, verified by mutation — never by code review alone).
//
...
// The SUBPROOF/PROOF consistency-PROOF *construction* below calls only those
// primitives — proof construction, not a new hash algorithm (identical
// allowance REGISTRY-LINEAGE-TILES-BUILD-1 used; re-derived independently
// here rather than imported, because §3.2 treats the two logs as separate
// mechanisms sharing only the primitives module, never a shared code path
// that would couple their tree sizes).
//
// ⛔ Sitemap: registry/errata/** is machine-fetched binary tile data, not a
// human-readable page, and is DELIBERATELY EXCLUDED from sitemap.xml (spec
// §5.3, SO #19) — same decision REGISTRY-LINEAGE-TILES-BUILD-1 recorded for
// registry/lineage/**. `registry` is NOT added to scripts/published-dirs.json.
//
// ⛔ No pruning, ever (§3.3): a full tile's bytes never change once written; a
// partial tile is superseded by a NEW file at a NEW `.p/<W>` path as the
// group fills in, and the old `.p/<W>` file is left untouched.
//
// ⛔ Two logs, one mechanism, NEVER merged (§3.2): this file and
// gen-registry-lineage.mjs are independent generators over independent input
// files, independent checkpoints, independent Sigsum leaves. Do not import
// state from one into the other.
//
// Zero-dep (CONTRACT.md): Node builtins + WebCrypto only.
//
// Usage:
//   node scripts/gen-registry-errata.mjs            # generate + publish + Sigsum-anchor
//   node scripts/gen-registry-errata.mjs --check     # read-only recompute-and-verify (preflight)
...
// Workspace root holds the OUT-OF-REPO key material (research/*.priv.jwk.json,
// never committed — same internal-artifact fence as every other Sigsum/log
// signing key in this estate). `resolve(REPO, '..')` only finds it when REPO
// IS the top-level checkout; it is one level too shallow for a worktree
// nested under `<workspace>/.wt/<branch>` (SO #3's own required location),
// which resolves to `<workspace>/.wt` instead. Walk up from REPO looking for
// the actual workspace root (marked by sibling `research/` + `CLAUDE.md`
// directories) instead of assuming a fixed nesting depth; fall back to the
// one-level assumption if nothing is found within a few levels (e.g. a
// checkout with no workspace-root scratch dir at all).
function findWorkspaceRoot(startDir) {
  let dir = startDir;
  for (let i = 0; i < 6; i++) {
    const parent = resolve(dir, '..');
    if (existsSync(join(parent, 'research')) && existsSync(join(parent, 'CLAUDE.md'))) return parent;
    if (parent === dir) break;
    dir = parent;
  }
  return resolve(startDir, '..');
}
const WORKSPACE_ROOT = findWorkspaceRoot(REPO);
const RECORDS_PATH = resolve(REPO, 'errata.json');
const REGISTRY_DIR = resolve(REPO, 'registry/errata');
...
const LOG_PRIVATE_KEY_PATH = resolve(WORKSPACE_ROOT, 'research/registry-errata-log-key.priv.jwk.json');
...
// DNS TXT record at _sigsum_v1.ainumbers.co. Same fix REGISTRY-LINEAGE-RETRY-1
// applied to gen-registry-lineage.mjs for the sibling log.
...
const ORIGIN = 'ainumbers.co/registry/errata';
const LOG_NAME = ORIGIN; // note-signature identifier, same string as the checkpoint origin
const LOG_PUBLIC_KEY_HEX = 'c824d4cd8d69ac0251c74e420889a84997ff024e360d8cf80bd861f8db447be7';
const TILE_WIDTH = 256; // C2SP tlog-tiles: 256 hashes / 8192 bytes per full tile

// BUILD-SPEC §2.2, inherited VERBATIM from FV-AGENTSURFACE-SPEC-1 §2. Never
// re-derive, never paraphrase — a paraphrase drifts silently, a copy drifts
// visibly. Published as a checkpoint extension line (see generate() below).
export const STALENESS_WORDING =
  'staleness: Past expires_at, treat it as unknown-refresh-needed — not as continuing to hold, and not as false either.';
...
// no partial group at all, and count===0 produces zero groups) — §3.3's
...
// of two. A level is only built while a FULL group of 256 lower-level
// entries exists.
// ---------------------------------------------------------------------------
...
// FAILING state (SO #34c), never treated as zero leaves found. size===0
// requires no files at all (tileGroups(0) === []).
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// RFC 6962 empty-tree root: MTH({}) = SHA-256() — the hash of the empty
// octet string. `sha256()` (variadic, zero args -> concatBytes([]) -> empty
// input) already computes exactly this; named here so callers read the
// intent rather than a bare zero-arg call.
// ---------------------------------------------------------------------------
export async function emptyTreeRoot() {
  return sha256();
}
...
export function loadEntries() {
  ...
  if (!Array.isArray(parsed.entries)) throw new Error(`${RECORDS_PATH}: expected a top-level "entries" array`);
...
// command (never duplicated), mirroring gen-registry-lineage.mjs's own
...
function submitCheckpointToSigsum(checkpointBytes) {
  return (async () => {
    ...
    const outPath = resolve(WORKSPACE_ROOT, `research/registry-errata-sigsum-record-${Date.now()}.json`);
    const args = [
...
export async function generate(opts = {}) {
  ...
  const entryByteArrays = entries.map(canonicalEntryBytes);
  const leafHashes = await Promise.all(entryByteArrays.map((b) => hashLeafNode(b)));
  const size = leafHashes.length;
  const newRoot = size === 0 ? await emptyTreeRoot() : await mth(leafHashes, 0, size);
  ...
  if (size < oldSize) throw new Error(`new entry count (${size}) is SMALLER than the previously published size (${oldSize}) — append-only violated, refusing to publish.`);

  const oldRoot = oldSize > 0 ? await mth(readLevel0LeafHashes(registryDir, oldSize), 0, oldSize) : await emptyTreeRoot();
  ...
  if (!consistent) {
    throw new Error(
      ...
      `Refusing to publish — do not regenerate harder, investigate the entry ordering/canonicalization.`
    );
  }
  console.log(`✓ consistency gate: new tree (size ${size}) verifiably extends old tree (size ${oldSize}).`);

  // ── Build tiles + entry bundles in memory (empty at size 0 — no files). ──
  const levels = size === 0 ? [] : await computeLevels(leafHashes);
  const filesToWrite = [];
  for (let L = 0; L < levels.length; L++) filesToWrite.push(...levelTileFiles(registryDir, L, levels[L]));
  if (size > 0) filesToWrite.push(...entriesBundleFiles(registryDir, entryByteArrays));

  // ── Checkpoint: format (with §2.2 staleness extension line), sign,
  // self-parse round-trip BEFORE anchoring ─────────────────────────────────
  const checkpointBody = formatCheckpoint(ORIGIN, size, newRoot, [STALENESS_WORDING]);
  ...
  if (!parsed.extensionLines.includes(STALENESS_WORDING)) {
    throw new Error('checkpoint self-parse: §2.2 staleness extension line missing after round-trip — refusing to publish.');
  }
  ...
  console.log('✓ checkpoint self-parse round-trip via parseSignedNote: origin/size/root/staleness-extension/signature all confirmed.');
  ...
  mkdirSync(registryDir, { recursive: true }); // needed unconditionally: a genesis (size=0) publish has zero tile files but still writes the checkpoint
  ...
  console.log(`\n✅ published registry/errata: size=${size} root=${bytesToHex(newRoot)}`);
...
export async function check(opts = {}) {
  ...
  const fail = (msg) => {
    console.error(`✗ gen-registry-errata --check: ${msg}`);
  ...
  if (!parsed.extensionLines.includes(STALENESS_WORDING)) {
    return fail(`published checkpoint is missing the BUILD-SPEC §2.2 staleness extension line verbatim.`);
  }
  ...
  const recomputedRoot = parsed.size === 0 ? await emptyTreeRoot() : await mth(leafHashes, 0, parsed.size);
  if (!bytesEqual(recomputedRoot, parsed.rootHash)) {
    return fail(
      `root recomputed from published tile bytes ` +
      `(${bytesToHex(recomputedRoot)}) does NOT match the published ` +
  ...
  const okMsg = `checkpoint signature valid, staleness wording present, ${entryBytes.length} entries cross-checked, root recomputed from published tile bytes matches the checkpoint (size=${parsed.size}, root=${bytesToHex(recomputedRoot)}).`;
  console.log(`✓ gen-registry-errata --check PASSED: ${okMsg}`);
