...
### TOOLPAGE-FILE-IMPORT-1 (2026-09-05) — the web-app manifest is not client storage, and `share_target` text routing is not egress

`manifest.webmanifest` is a static site descriptor the BROWSER reads (install metadata, icon, start URL); it is not a storage mechanism — nothing is written to the device on its behalf, and the estate still ships no service worker, so no offline cache, no background work, and no storage origin exists. The `share_target` member declares that when a user (or a local automation) shares plain TEXT into the installed app, the browser hands that text to `/start.html?share=…` as a normal local navigation — no network request leaves the machine beyond the navigation to the user's own already-loaded origin, which is the same zero-egress surface as typing in the search box by hand. `start.html` uses `?share=` ONLY to prefill the local tool-search routing box and never as tool input, preserving the deep-link contract's fragment-only rule for parameters (TOOLPAGE-DEEPLINK-1). Likewise the node-page file-import reader (drag-drop / file picker) keeps file bytes in memory only: no storage API is written, no upload occurs, and the sole accepted doorway is the same in-page validation and prefill path the fragment deep link uses.
