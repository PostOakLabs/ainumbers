...
- **No comma-splice fragment headings:** a visible `h1`-`h6` that joins two or more verbless fragments with commas and no conjunction ("One formula, four checks") reads as machine-generated cadence. Rewrite as a natural heading ("The formula and its four checks"), a colon form ("AIN Bridge: the browser-side wiring layer"), or a conjoined list ("generate, publish, sign, verify, and rotate"). Coordinate lists with a conjunction, tag lists of standards/acronyms ("PD, LGD, EAD…"), parentheticals, and trailing `ART-xx`/`T-xxx` node-id suffixes are exempt; ", not X" headings belong to the tic rule above. Baselined legacy headings ratchet down; new pages must be clean. (HALLMARKS-FRAGMENT-HEADING-GATE, 2026-09-22.)
