...
- **TOOLIFY tool-to-node pairings (TF-3, 2026-07-18):** each browser tool below keeps its page (UI) AND gained a ChainGraph node (proof) — intentional duplication, not drift. Cross-linked both directions. T203 and T437 (`raroc-loan-pricing-model` / `raroc-loan-pricing`) &rarr; ART-362 `compute_raroc_loan_price`. T438 (`dscr-interest-coverage-calculator`) &rarr; ART-363 `compute_dscr`. T469/T470/T471 (`lcr-calculator` / `nsfr-calculator` / `leverage-ratio-calculator`) &rarr; ART-364 `compute_lcr_nsfr_leverage`. T473/T474 (`globe-etr-jurisdiction-calculator` / `topup-tax-qdmtt-calculator`) &rarr; ART-365 `compute_globe_topup_tax`. T446 (`embedded-insurance-pricing-modeller`) &rarr; ART-366 `price_embedded_insurance`. T141 (`cross-border-b2b-fee-calculator`) &rarr; ART-367 `compute_cross_border_fees`. T105 (`fx-netting-simulator`) &rarr; ART-368 `compute_fx_netting_positions`. **Special case:** T457 (`naic-rbc-calculator`) pairs with the PRE-EXISTING ART-254 `compute_rbc_action_level` — no new kernel was built since ART-254 already computes that ratio.
