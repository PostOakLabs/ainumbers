...
- **Scope:** Translates UI chrome only. Regulatory citations, ISO/SWIFT codes, JS logic, AP2 keys, and tool-generated output remain in English.
...
- `mandate_type` lives **in the AP2 payload**, NOT in `manifest.json`.
...
## 📦 3. AP2 Export Schema & UI Contract
### 3.1 Canonical AP2 Mandate v1.0 Schema
...
- **State Management:**
  ...
  |---|---|---|
  | Pre-Run | `disabled`, `opacity: 0.28`, `cursor: not-allowed` | Tooltip: `"Run the analysis above to generate an AP2 export"` |
...
- **File Naming:** `{tool_id}_{YYYYMMDDHHMMSS}.ap2.json`
...
|---|---|---|
| **Tier 1 (Mandatory)** | `AP2 JSON` + `Markdown` | All policy, rule, mandate, routing, compliance, or calculator tools |
...
# Verify AP2 schema compliance on generated payloads
...
- [ ] AP2 button present where `ap2_export: true`, validates before download
...
function exportAP2() {
  // 1. Validate payload against AP2Schema
